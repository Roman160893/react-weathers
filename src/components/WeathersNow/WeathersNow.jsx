...
const WeathersNow = ({ weatherNow, dateNow }) => {
  const weatherNowArr = useMemo(
    () => [
      {
        value: weatherNow.clouds.all,
        icon: faCloud,
        description: "Хмарність",
        unit: "%",
      },
      {
        value: weatherNow.wind.speed,
        icon: faWind,
        description: "Швидкість вітру",
        unit: "м/с",
      },
      {
        value: weatherNow.main.humidity,
        icon: faDroplet,
        description: "Вологість",
        unit: "%",
      },
      {
        value: Math.ceil(weatherNow.main.feels_like),
        icon: faGaugeHigh,
        description: "Відчувається як",
        unit: "°C",
      },
      {
        value: weatherNow.main.pressure,
        icon: faMeteor,
        description: "Атмосферний тиск",
        unit: "hPa",
      },
      {
        value: weatherNow.visibility * 0.001,
        icon: faEyeSlash,
    ...
    <div className="main-page">
      <div className="main-content__main big">
        <div>
          {weatherNow.weather.map((el, index) => {
            return (
              <div className="main-content__icon" key={index}>
                <img src={getImage(el.icon)} />
                <p>{`${el.description[0].toUpperCase()}${el.description
                  .slice(1)
                  .toLowerCase()}`}</p>
              </div>
            );
          ...
          <div className="main-content__date">{dateNow.date}</div>
      ...
      <div className="main-content__other">
        {weatherNowArr.map((obj, index) => (
          <WeatherDescription key={index} {...obj} />
        ))}
      </div>
    </div>
  );
